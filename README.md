...
- **Start my day** with available-hours inputs that preserve half-hour budgets, a compact multi-line manual-items box, minute-aligned local clock context, and an editable Preview that includes local date, time, timezone, ISO week, quarter, exact target notes, and a compact boundary-artifact check note; its daily guard gives concrete targets and catches up missing weekly/monthly/quarterly artifacts after the relevant period boundary has passed, weekly shortcuts expose a compact Week field for `YYYY-WX`, `YYYY-WXX`, or `last` with week numbers normalized before preview, annual shortcuts expose a visible year field where blank/last/next resolve per run without sticking to the shared field, quarterly/monthly shortcuts expose a compact Period field for `YYYY-QX`, `YYYY-MM`, or bare `MM`, deriving quarters from `YYYY-MM` for plan/review and mapping `YYYY-QX` to that quarter's final monthly pulse before Preview, while advanced prompts preserve raw slash commands except for target shorthand normalization on predicted weekly, annual, and monthly pulse outputs
...
- Native **Quick Capture** for ideas, tasks, meeting notes, and research questions without leaving Operator Home; pasted multi-line captures become separate Markdown items
