...
- **Start my day** with available-hours inputs that preserve half-hour budgets, manual-items input, minute-aligned local clock context, and an editable Preview that includes local date, time, timezone, ISO week, quarter, exact target notes, and a compact boundary-artifact check note; its daily guard gives concrete targets and catches up missing weekly/monthly/quarterly artifacts after the relevant period boundary has passed, weekly shortcuts expose a compact Week field for `YYYY-WXX` or `last` with `last` resolved before preview, annual shortcuts expose a visible year field where blank/last/next resolve per run without sticking to the shared field, quarterly/monthly shortcuts expose a compact Period field for `YYYY-QX`, `YYYY-MM`, or bare `MM` with month-only pulse targets resolved to concrete `YYYY-MM` before Preview, while advanced prompts preserve raw slash commands
...
- Backend-specific disabled-state help beside **Start my day** and inside **More workflows**, so locked agent buttons say whether Codex or Claude is missing CLI, login, skills, or vault setup
